...
// 用来绘图的颜色
#define COLOR_SNAKE			RGB(75,0,130)
#define COLOR_FOOD			RGB(255,182,193)
...
void GamePaint(HWND hwnd)
{
	...
	*******************************************************************************/
	...
	//hPenBoundary = CreatePen(PS_SOLID, 19, COLOR_BOUNDARY);
	//hPenBoundary = CreateSolidBrush(COLOR_BOUNDARY);
	hPenBoundary = CreateHatchBrush(HS_CROSS, RGB(160, 82, 45));
	//BkgBrush= CreateSolidBrush(RGB(124, 252, 0));
	...
	*******************************************************************************/
	...
	for (i = 0; i < 56; i++)
	{
		...
		Rectangle(hdcmem,
			pWallBody->x * CELL_PIXEL + rectBoundary.left,
			pWallBody->y * CELL_PIXEL + rectBoundary.top,
			(pWallBody->x + 1)*CELL_PIXEL + rectBoundary.left,
			(pWallBody->y + 1)*CELL_PIXEL + rectBoundary.top);
	}
	...
	*******************************************************************************/
	...
	for (i = 0; i < block_size; i++)
	{
		...
		Rectangle(hdcmem,
			...
			get_block_y(pBlockBody) * CELL_PIXEL + rectBoundary.top,
			(get_block_x(pBlockBody)+ 1)*CELL_PIXEL + rectBoundary.left,
			(get_block_y(pBlockBody) + 1)*CELL_PIXEL + rectBoundary.top);
	}
	...
	*******************************************************************************/
	...
	if (hOldFont = (HFONT)SelectObject(hdcmem, hFont))
	{
		...
		wsprintf(szSourceInfo, "score %d level %d ResetTimes %d", GetScore(), GetLevel(), GetResetTimes());
		// 设置输出颜色
